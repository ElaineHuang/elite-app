...
def simulation_restore_m():
	...
	machine_learning_model = os.path.join(data_path, 'machine_learning/')
	for machine in Machines:
		...
		try:
			...
			#步驟2 創建layer
			input_layer=add_layer(xs, 53, 9, n_layer = 1, activation_function = tf.sigmoid)
			hidden_layer1=add_layer(input_layer, 9, 9, n_layer = 2, activation_function = tf.sigmoid)
			hidden_layer2=add_layer(hidden_layer1, 9, 9, n_layer = 3, activation_function = tf.nn.softmax)
			output_layer=add_layer(hidden_layer2, 9, 1, n_layer=4, activation_function = tf.nn.relu6)
			...
			writer = tf.summary.FileWriter(machine_learning_model + "logss/", sess.graph)
			...
			# 如果now_status_predict超過1變1 小於-1變-1
			now_status_predict=np.fmin(1.0, np.fmax(0.0, now_status_predict)) 
			arti_weight2 = artificial * now_status_in
			arti_sum2 = np.sum(arti_weight2,axis=1)  #axis=1代表同列相加
			arti_sum2 = np.reshape(arti_sum2,(len(arti_sum2),1))

			arti_sum2=np.fmin(1.0, np.fmax(0.0, arti_sum2)) 

			now_status_predict = now_status_predict * 50 + arti_sum2 * 50
			...
			remove_files = glob.glob(machine_learning_model + 'logss/*')
