...
SMT_path = 'D:\Documents\IoT\preprocessing\SMT'
files = [f for f in os.listdir(SMT_path) if os.path.isfile(os.path.join(SMT_path, f))]
for f in files:
	fileName = os.path.splitext(f)[0]
	ErrRec = open(fileName+"_file.csv","w")
	ErrRec.write("StartTime,ErrCode,Repetition\n")
	unsortedTable = []
	workbook = openpyxl.load_workbook(filename=os.path.join(SMT_path, f), read_only=True)
